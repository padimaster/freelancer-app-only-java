...
public class AuthServiceTest {
  ...
  @Test
  public void shouldValidateUser() throws InvalidCredentialsException, NotFoundException {
    UserEntity user;
    ...
    user = this.authService.validateUser(expectedUser.getId(), expectedUser.getPassword());

    assertEquals(user, expectedUser);
  }

  @Test(expected = InvalidCredentialsException.class)
  public void shouldNotValidateUser() throws InvalidCredentialsException, NotFoundException {
    UserEntity user;
    UserEntity notExpectedUser;

    notExpectedUser = new UserEntity("999999", "Alex Padilla", "alex@mail.com", "222222");

    Mockito.when(usersService.getById(anyString())).thenReturn(notExpectedUser);

    user = this.authService.validateUser(notExpectedUser.getId(), "randomPassword");
